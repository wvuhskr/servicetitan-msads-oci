...
from st_msads_oci.rows import UploadRow, LEGACY_GOALS

GOAL_BOOKINGS = LEGACY_GOALS[0]  # "ServiceTitan Integrated Bookings - MS" — legacy, still in old result files
...
def test_run_triage_updates_statuses_and_history(tmp_path):
    ...
    findings = run_triage(ledger, inbox, downloads, archive, history, parsed_on="2026-07-06")
...
def test_zip_result_file_is_archived_and_not_reprocessed(tmp_path):
    ...
    # Run 1
    ledger1 = Ledger()
    ledger1.uploaded = seed_from_result_csv(JUNE)
    for e in ledger1.uploaded:
        e["status"] = None
    findings1 = run_triage(ledger1, inbox, downloads, archive, history, parsed_on="2026-07-06")
    ...
    ledger2 = Ledger()
    ledger2.uploaded = seed_from_result_csv(JUNE)
    for e in ledger2.uploaded:
        e["status"] = None
    findings2 = run_triage(ledger2, inbox, downloads, archive, history, parsed_on="2026-07-06")
