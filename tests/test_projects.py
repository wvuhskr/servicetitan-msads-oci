...
def test_canonical_two_leg_project():
    ...
    rows, members, findings, dropped = project_rows_from_payload(
        payload({110000002: [lead, install]}), Ledger(), NOW, TODAY, "Paid Microsoft")
...
def test_open_job_pends_project():
    ...
    rows, members, findings, dropped = project_rows_from_payload(
        payload({500: [lead, install]}), ledger, NOW, TODAY, "Paid Microsoft")
...
def test_originless_project_withheld_as_suspect_but_pends():
    ...
    rows, members, findings, dropped = project_rows_from_payload(
        payload({600: jobs}), ledger, NOW, TODAY, "Paid Microsoft")
    ...
    project_rows_from_payload(payload({600: jobs}), ledger, NOW, "2026-08-04", "Paid Microsoft")
    assert ledger.pending_projects == [{"project_id": 600, "first_seen": TODAY}]
...
def test_suspect_project_uploads_once_lead_origin_appears():
    ...
    rows, _, findings, dropped = project_rows_from_payload(
        payload({600: [job(100, **LEAD), job(101, total=6729.0, est=4)]}), ledger, NOW, TODAY, "Paid Microsoft")
    assert findings == [] and dropped == []
    [r] = rows
...
def test_member_already_in_ledger_blocks_project():
    ...
    rows, members, findings, dropped = project_rows_from_payload(
        payload({700: [lead, install]}), ledger, NOW, TODAY, "Paid Microsoft")
...
def test_late_revenue_reported_never_reuploaded():
    ...
    rows, members, findings, dropped = project_rows_from_payload(
        payload({800: [lead, install, third]}), ledger, NOW, TODAY, "Paid Microsoft")
    assert rows == [] and dropped == []
...
def test_late_revision_detector_flags_upward_revision():
    ...
    rows, _, findings, dropped = project_rows_from_payload(
        payload({810: [lead, install]}), ledger, NOW, TODAY, "Paid Microsoft")
    assert rows == [] and dropped == []
...
def test_late_revision_detector_skips_legacy_entry_without_value():
    ...
    rows, _, findings, dropped = project_rows_from_payload(
        payload({820: [lead, install]}), ledger, NOW, TODAY, "Paid Microsoft")
    assert rows == [] and dropped == []
...
def test_malformed_project_job_finding_and_pending():
    ...
    rows, members, findings, dropped = project_rows_from_payload(p, ledger, NOW, TODAY, "Paid Microsoft")
    [r] = rows
...
def test_window_expired_skipped_and_reported():
    ...
    rows, _, findings, _ = project_rows_from_payload(
        payload({900: [lead, install]}), Ledger(), NOW, TODAY, "Paid Microsoft")
    assert rows == []
...
def test_lead_only_project_uploads():
    ...
    rows, _, findings, dropped = project_rows_from_payload(
        payload({1000: [lead]}), Ledger(), NOW, TODAY, "Paid Microsoft")
    [r] = rows
...
def test_canceled_fulfillment_excluded_not_blocking():
    ...
    rows, _, findings, dropped = project_rows_from_payload(
        payload({1100: [lead, canceled]}), Ledger(), NOW, TODAY, "Paid Microsoft")
    [r] = rows
...
def test_settled_zero_revenue_dropped():
    ...
    canceled = job(101, status="Canceled", total=8000.0)
    rows, _, findings, dropped = project_rows_from_payload(
        payload({1200: [lead, canceled]}), Ledger(), NOW, TODAY, "Paid Microsoft")
    assert rows == []
...
def test_lead_not_completed_withheld():
    ...
    install = job(101, total=5000.0)
    rows, _, findings, _ = project_rows_from_payload(
        payload({1300: [lead, install]}), Ledger(), NOW, TODAY, "Paid Microsoft")
    assert rows == []
...
def test_earliest_created_lead_wins():
    ...
    [r], _, _, _ = project_rows_from_payload(
        payload({1400: [late, early]}), Ledger(), NOW, TODAY, "Paid Microsoft")
    assert r.st_id == 100
...
def test_google_lead_row_carries_google_campaign():
    ...
    [r], _, _, _ = project_rows_from_payload(
        payload({1500: [lead, install]}), Ledger(), NOW, TODAY, "Paid Microsoft")
...
def test_missing_project_jobs_reported_and_pended():
    ...
    p = {"campaigns": CAMPS,
         ...
         "project_jobs": {}}
    rows, members, findings, _ = project_rows_from_payload(p, ledger, NOW, TODAY, "Paid Microsoft")
    assert rows == []
...
def test_pending_project_rechecked_without_new_completed_jobs():
    ...
    rows, _, findings, dropped = project_rows_from_payload(p, ledger, NOW, TODAY, "Paid Microsoft")
    [r] = rows
...
def test_no_projects_in_payload_is_a_noop():
    ...
    assert project_rows_from_payload(p, Ledger(), NOW, TODAY, "Paid Microsoft") == ([], set(), [], [])
...
def test_nonms_suspect_reported_once_and_dropped():
    ...
    rows, _, findings, dropped = project_rows_from_payload(
        payload({600: jobs}), ledger, NOW, TODAY, "Paid Microsoft")
    assert rows == [] and dropped == []
    assert any("non-Paid Microsoft campaign" in f for f in findings)
    assert ledger.pending_projects == []
...
def test_call_inference_recovers_from_non_proxy_leg():
    ...
    rows, _, findings, dropped = project_rows_from_payload(
        payload({600: jobs}, calls=calls), ledger, NOW, TODAY, "Paid Microsoft")
    assert dropped == []
    [r] = rows
...
def test_suspect_call_inference_earliest_qualifying_call_wins():
    ...
    rows, _, findings, _ = project_rows_from_payload(
        payload({600: SUSPECT_JOBS}, calls=calls), Ledger(), NOW, TODAY, "Paid Microsoft")
    [r] = rows
    assert r.recovered_via == "call"
    assert r.lead_call_id == 8
